...
터미널 환경에서 `pipeline/prompts/`와 `pipeline/templates/`를 파이프(`|`)로 연결하여 사용하는 실무 가이드.
...
> tier별 timeout 및 모델 매핑은 `pipeline/scripts/lib/ai.sh`와 `pipeline/scripts/providers/claude.sh`에서 관리한다.
...
printf '%s\n\n---\n\n## 이슈 정보\n[no-issue] 작업 내용: 로깅 추가\n\n## 기존 브랜치 목록\n%s\n' \
  "$(cat ax-driven/pipeline/prompts/05-branch-name-guide.md)" \
...
git diff --cached | cat ax-driven/pipeline/prompts/00-git-commit-guide.md - | claude --print --model sonnet > ax-driven/tmp/commit.md
...
명세서는 `pipeline/templates/03-work-specification.md` 템플릿을 기반으로 작성한다.
...
`pipeline/templates/03-work-specification.md` 템플릿을 기반으로 작성하는 것을 권장한다.
