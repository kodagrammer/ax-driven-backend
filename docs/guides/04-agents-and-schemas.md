...
- **Retrieval 범위**: reviewer가 보는 컨텍스트는 기본적으로 diff에 한정된다. import/symbol depth 탐색 같은 Phase 2 retrieval은 future direction이며, 본 레포 마무리 작업으로 변경 파일 일부를 인라인하는 MVP만 추가한다.
- **Severity 처리**: 현재는 reviewer 출력의 `severity`를 그대로 사용한다. verification_status × confidence 매트릭스 같은 풀 가드는 도입하지 않는다.
